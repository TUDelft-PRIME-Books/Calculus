...
<body><main class="page">
  <h1 class="headline">Book progress</h1><p class="subtitle">Section-level writing, review, pilot, and completion status.</p><div class="source">Source: <strong>Details</strong> tab · <span id="sourceN"></span> sections</div>
  ...
  <section class="grid"><article class="card"><div class="title">Pilot coverage</div><p class="desc">Sections currently assigned to each pilot group, with average progress.</p><div class="pilot-grid" id="pilots"></div></article><article class="card insight"><div class="title">Portfolio signals</div><p class="desc">Additional data-driven observations from the currently selected scope.</p><ul id="signals"></ul></article></section>
...
const $=id=>document.getElementById(id); const pct=n=>Math.round(n)+'%';
function esc(s){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
function opts(){let ch=[...new Map(DATA.rows.map(r=>[r.chapter,r.chapterName])).entries()];$('chapterFilter').innerHTML='<option value="all">All chapters</option>'+ch.map(([n,x])=>`<option value="${n}">Ch. ${n} — ${esc(x)}</option>`).join('');$('stageFilter').innerHTML='<option value="all">All stages</option>'+stages.map(s=>`<option value="${esc(s)}">${esc(s)}</option>`).join('');let ps=[...new Set(DATA.rows.map(r=>r.pilot))];$('pilotFilter').innerHTML='<option value="all">All pilot groups</option>'+ps.map(s=>`<option value="${esc(s)}">${esc(s)}</option>`).join('');}
function view(){const c=$('chapterFilter').value,s=$('stageFilter').value,p=$('pilotFilter').value;return DATA.rows.filter(r=>(c==='all'||String(r.chapter)===c)&&(s==='all'||r.stage===s)&&(p==='all'||r.pilot===p));}
function weights(stage){return {'Not started':0,'Early stages':10,'Ready for review 1':30,'Review 1 completed':50,'Ready for review 2':70,'Review 2 completed':90,'Finished':100}[stage]||0}
function render(){const rows=view(), n=rows.length||0, weighted=n?rows.reduce((a,r)=>a+weights(r.stage),0)/n:0, fin=rows.filter(r=>r.stage==='Finished').length, ready=rows.filter(r=>['Ready for review 1','Review 1 completed','Ready for review 2','Review 2 completed','Finished'].includes(r.stage)).length;$('sourceN').textContent=DATA.n;$('kpiN').textContent=n;$('kpiWeighted').textContent=pct(weighted);$('explainerPct').textContent=pct(weighted);$('kpiFinished').textContent=fin;$('kpiFinishedHint').textContent=`${n?pct(100*fin/n):'0%'} of selected sections`;$('kpiReady').textContent=ready;
$('legend').innerHTML=stages.map(s=>`<span class="legend-item"><i class="dot" style="background:${colors[s]}"></i>${esc(s)}</span>`).join('');
...
let cps=gs.map(([k,g])=>{const [cn,name]=k.split('|');let v=g.reduce((a,r)=>a+weights(r.stage),0)/g.length;return {lab:`Ch. ${cn} — ${name}`,v}});$('chapterProgress').innerHTML=cps.length?cps.map(x=>`<div class="prog-row"><div class="prog-label" title="${esc(x.lab)}">${esc(x.lab)}</div><div class="prog-track"><div class="prog-fill" style="width:${x.v}%"></div></div><div class="prog-val">${pct(x.v)}</div></div>`).join(''):'<div class="desc">No chapters in view.</div>';
let amap={};rows.forEach(r=>(amap[r.author]??=[]).push(r));let as=Object.entries(amap).map(([a,g])=>({a,n:g.length,v:g.reduce((x,r)=>x+weights(r.stage),0)/g.length})).sort((a,b)=>b.v-a.v);$('authors').innerHTML=as.length?as.map(x=>`<div class="author-row"><div class="author-name">${esc(x.a)}</div><div><div class="author-bar"><div class="author-fill" style="width:${x.v}%"></div></div></div><div class="author-stat">${x.n} · ${pct(x.v)}</div></div>`).join(''):'<div class="desc">No owners in view.</div>';
let pmap={};rows.forEach(r=>(pmap[r.pilot]??=[]).push(r));$('pilots').innerHTML=Object.entries(pmap).sort((a,b)=>b[1].length-a[1].length).map(([p,g])=>{let w=g.reduce((x,r)=>x+weights(r.stage),0)/g.length;return `<div class="pilot"><div class="pname">${esc(p)}</div><div class="pval">${g.length}</div><div class="small">${pct(100*g.length/n)} of scope · ${pct(w)} weighted</div></div>`}).join('')||'<div class="desc">No pilot assignments in view.</div>';
