...
::::::{prf:example}
$y''-y'-6y=0$ has characteristic equation $r^2-r-6=0\;\Longleftrightarrow\;(r-3)(r+2)=0$ with solutions $r_1=3$ and $r_2=-2$. Hence, the general solution is $y=c_1e^{3x}+c_2e^{-2x}$ with $c_1,c_2\in\mathbb{R}$.
::::::
...
```{math}
ay''+by'+cy=a(r^2x+2r)e^{rx}+b(rx+1)e^{rx}+cxe^{rx}=(ar^2+br+1)xe^{rx}+(2ar+b)e^{rx}=0+0=0.
```
Since $y_1=e^{rx}$ and $y_2=xe^{rx}$ are linear independent, the general solution is $y=c_1e^{rx}+c_2xe^{rx}$ with $c_1,c_2\in\mathbb{R}$.

::::::{prf:example}
$y''-6y'+9y=0$ has characteristic equation $r^2-6r+9=0\;\Longleftrightarrow\;(r-3)^2=0$ with 'double' solution $r=3$. Hence, the general solution is $y=c_1e^{3x}+c_2xe^{3x}$ with $c_1,c_2\in\mathbb{R}$.
::::::
...
```{math}
e^{(\alpha+i\beta)x}=e^{\alpha x}\cdot e^{i\beta x}=e^{\alpha x}(\cos(\beta x)+i\sin(\beta x))\quad\text{and}\quad e^{(\alpha-i\beta)x}=e^{\alpha x}\cdot e^{-i\beta x}=e^{\alpha x}(\cos(\beta x)-i\sin(\beta x)).
```
...
::::::{prf:example}
$y''+4y'+13y=0$ has characteristic equation $r^2+4r+13=0\;\Longleftrightarrow\;(r+2)^2+9=0$ with nonreal solutions $r=-2\pm3i$. Hence, the general solution is $y=c_1e^{-2x}\cos(3x)+c_2e^{-2x}\sin(x)$ with $c_1,c_2\in\mathbb{R}$.
::::::

<b>Initial-value problems</b>

An initial-value problem for a homogeneous or nonhomogeneous second-order differential equation consists of finding a solution $y$ of the differential equation that also satisfies initial conditions of the form $y(x_0)=y_0$ and $y'(x_0)=y_1$, where $y_0$ and $y_1$ are given constants. If $P$, $Q$, $R$ and $G$ are continuous on an interval $I$ with $P(x)\neq0$ and $x_0\in I$, then a theorem found in more advanced books guarantees the existence and uniqueness of a solution of such an initial-value problem.
