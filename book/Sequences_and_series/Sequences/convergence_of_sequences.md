...
From {prf:ref}`Ex:Sequences:IncreasingExample1Function`, {prf:ref}`Ex:Sequences:BoundedExample1` and {prf:ref}`Ex:Sequences:ConvergenceExampleIncreasing` we can conclude that the sequence $\{a_n\}$ with $a_n=\dfrac{n}{n+1}$ is increasing, bounded above (by $1$) and convergent. From {prf:ref}`Ex:Sequences:IncreasingExample2Function`, {prf:ref}`Ex:Sequences:BoundedExample2` and {prf:ref}`Ex:Sequences:ConvergenceExampleDecreasing` we can conclude that the sequence $\{a_n\}$ with $a_n=\dfrac{2n}{n^2+1}$ is decreasing, bounded below (by $0$) and convergent.
...
We have seen that the sequence decreases and is bounded below by $0$. This implies that the sequence converges. So, the limit $\lim\limits_{n\to\infty}a_n$ exists.
