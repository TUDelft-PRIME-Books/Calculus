...
As you can see, the terms of the sequence $\{a_n\}$ seem to keep going up, the terms of the sequence $\{b_n\}$ seem to keep going down, but are staying above zero, and the terms of the sequence $\{c_n\}$ seem to switch between two values. These observations can also be described using some terminology, which we will define next:

::::{prf:definition}
:label: Def:PropertiesOfSequences:Affirming

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **strictly increasing** if $a_{n+1} > a_n$ for all $n\geq p$.

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **strictly decreasing** if $a_{n+1} < a_n$ for all $n\geq p$.

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **ultimately increasing** if there exists a number $N\geq p$ such that $a_{n+1} > a_n$ for all $n\geq N$.

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **ultimately decreasing** if there exists a number $N\geq p$ such that $a_{n+1} < a_n$ for all $n\geq N$.

::::

Next to the affirming definitions of {prf:ref}`Def:PropertiesOfSequences:Affirming`, we also have some invalidating definitions.

::::{prf:definition}
:label: Def:PropertiesOfSequences:Invalidating

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **non-increasing** if $a_{n+1} \leq a_n$ for all $n\geq p$.

A sequence $\{a_n\}_{n=p}^{\infty}$ is called **non-decreasing** if $a_{n+1} \geq a_n$ for all $n\geq p$.

::::::

Because of the similarity of the behaviour of non-decreasing and non-increasing sequences, we will often use the term *monotonic* to refer to both of these types of sequences:
...
As this difference is positive for all $n\in\{1,2,3,\ldots\}$, we conclude that the sequence $\{a_n\}_{n=1}^{\infty}$ is _strictly increasing_.
::::
...
&= \dfrac{5n^2}{n^2(n+1)^2} - \dfrac{5(n+1)^2}{n^2(n+1)^2} \\
&= \dfrac{5n^2 - 5(n+1)^2}{n^2(n+1)^2} \\
&= \dfrac{5n^2 - 5(n^2+2n+1)}{n^2(n+1)^2} \\
&= -\dfrac{10n+5}{n^2(n+1)^2}<0
\end{align*}

for all $n\in\{1,2,3,\ldots\}$.

We conclude that the sequence $\{b_n\}_{n=1}^{\infty}$ is _strictly decreasing_.
::::
...
We conclude that the sequence $\{c_n\}$ is *non-monotonic*, as it is neither non-increasing nor non-decreasing.
::::
...
- A *geometric* sequence is *strictly increasing* if its initial term is positive and the common ratio $r>1$, or if its initial term is negative and the common ratio $0<r<1$.

- A *geometric* sequence is *strictly decreasing* if its initial term is positive and the common ratio $0<r<1$, or if its initial term is negative and the common ratio $r>1$.

- Every *alternating sequence* is non-monotonic.
::::

For sequences that are not of such a common type, it sometimes is harder to show whether they are strictly increasing, strictly decreasing, monotonic or one of the other classifications, as the next examples show.

::::::{prf:example}
:label: Ex:Sequences:IncreasingExample

We show that the sequence $\{a_n\}_{n=1}^{\infty}$ with $a_n=\dfrac{n}{n+1}$ is strictly increasing.

To do so, we want to show that $\dfrac{n+1}{n+2} > \dfrac{n}{n+1}$ for all $n=1,2,3,\ldots$.

Since $n>0$ this is equivalent to showing that

$$
(n+1)^2 > n(n+2)\;\Longleftrightarrow\;n^2+2n+1 > n^2+2n.
$$

The last inequality reduces to $1 > 0$, which definitely is true. This shows that the sequence is *strictly increasing*.

::::::

::::::{prf:example}
:label: Ex:Sequences:DereasingExample

Consider the sequence $\{b_n\}_{n=1}^{\infty}$ with $b_n=\dfrac{2n}{n^2+1}$.

We will show that $\dfrac{2(n+1)}{(n+1)^2+1} < \dfrac{2n}{n^2+1}$ for all $n=1,2,3,\ldots$.

Note that

\begin{align*}
&~ & \dfrac{2n+2}{n^2+2n+2} & < \dfrac{2n}{n^2+1} \\
&\Longleftrightarrow & (2n+2)(n^2+1) & < 2n(n^2+2n+2) \\
&\Longleftrightarrow & 2n^3+2n^2+2n+2 & < 2n^3+4n^2+4n \\
&\Longleftrightarrow & 2 & < 2n^2+2n \\
&\Longleftrightarrow & 1 & < n(n+1).
\end{align*}

This is true for $n\geq1$, so the sequence is *strictly decreasing*.
...
Consider the sequence $\{c_n\}_{n=1}^{\infty}$ with $c_n=\dfrac{n^2}{2^n}$. Then the first terms are

$$
\frac{1}{2},\quad1,\quad\frac{9}{8},\quad1,\quad\frac{25}{32},\quad\frac{9}{16},\quad\frac{49}{128},\quad\ldots
$$

Note that $c_2-c_1=\frac{1}{2}>0$ and $c_3-c_2=\frac{1}{8}>0$, However, $c_4-c_3=-\frac{1}{8}<0$ and $c_5-c_4=-\frac{7}{32}<0$. So this sequence is non-monotonic, but it might be ultimately decreasing.

To investigate this, we consider the ratio between two consecutive terms:

\begin{align*}
\dfrac{c_{n+1}}{c_n} &= \dfrac{(n+1)^2}{2^{n+1}}\cdot\dfrac{2^{n}}{n^2} \\
...
&\Longleftrightarrow & \dfrac{(n+1)^2}{n^2} & < 2 \\
&\Longleftrightarrow & (n+1)^2 & < 2n^2 \\
&\Longleftrightarrow & n^2 + 2n + 1 & < 2n^2 \\
&\Longleftrightarrow & 2n + 1 & < n^2 \\
&\Longleftrightarrow & 0 & < n^2 - 2n - 1.
\end{align*}

The quadratic expression on the right of the last inequality would be zero if $n=1-\sqrt{2}\approx -0.414<0$ or if $n=1+\sqrt{2}\approx 2.414>0$. Since $n$ is a positive integer, we conclude that $n^2 - 2n - 1>0$ for all $n\geq3$. Hence, we have $\dfrac{1}{2}\cdot\dfrac{(n+1)^2}{n^2}<1$ for all $n\geq3$.
...
Let $\{a_n\}_{n=p}^{\infty}$ be a sequence and $f:\mathbb{R}\rightarrow\mathbb{R}$ a function such that $f(n)=a_n$ for $n=p,p+1,p+2,\ldots$

- If $f$ is a strictly increasing function for $x\geq p$, then $\{a_n\}_{n=p}^{\infty}$ is a strictly increasing sequence.

- If $f$ is a strictly decreasing function for $x\geq p$, then $\{a_n\}_{n=p}^{\infty}$ is a strictly decreasing sequence.

- If $f$ is a monotonic function for $x\geq p$, then $\{a_n\}_{n=p}^{\infty}$ is a monotonic sequence.

- If $f$ is a strictly increasing function for $x\geq N>p$ for some integer $N$, then $\{a_n\}_{n=p}^{\infty}$ is an ultimately increasing sequence (for $n\geq N$).

- If $f$ is a strictly decreasing function for $x\geq N>p$ for some integer $N$, then $\{a_n\}_{n=p}^{\infty}$ is an ultimately decreasing sequence (for $n\geq N$).
...
This implies that $f$ is strictly increasing and therefore that the sequence stricly increases too.
::::::

::::::{prf:example}
:label: Ex:Sequences:IncreasingExample2Function

We show that the sequence $\{b_n\}_{n=1}^{\infty}$ with $b_n=\dfrac{2n}{n^2+1}$ is decreasing.
...
Note that $b_n=g(n)$ if $g(x)=\dfrac{2x}{x^2+1}$. Then we have

$$
g'(x)=\frac{2(x^2+1)-2x\cdot2x}{(x^2+1)^2}=\frac{2(1-x^2)}{(x^2+1)^2},
$$

which is negative for $x>1$. This implies that $g$ is strictly decreasing on $(1,\infty)$ and so $g(n+1)<g(n)$. Therefore $\{b_n\}$ is strictly decreasing.
::::::

If a sequence is defined by a recursive formula, it is often somewhat more difficult to show any of the properties. However, not always, as in the next example:

::::::{prf:example}
:label: Ex:PropertiesOfSequences:Fibonacci

We revisit the Fibonacci sequence from {numref}`Sec:SeqAndTypes:Fibonacci`: $\{F_n\}_{n=1}^{\infty}$ with $F_1=F_2=1$ and 

$$
F_{n+2}=F_{n+1}+F_n\quad\text{for}\quad n=1,2,3,\ldots.
$$
...
Because we start with two positive numbers, and keep adding these positive numbers, every term in the Fibonacci sequence will be positive. This implies that $F_{n+2}-F_{n+1}$ is positive for all $n$ for which we are allowed to use the recursive formula.
...
In many other situations the {prf:ref}`principle of mathematical induction <Thm:Algebra:MathematicalInduction>` might be helpful:
...
We will apply the {prf:ref}`principle of mathematical induction <Thm:Algebra:MathematicalInduction>` to show that the sequence strictly increases.

To do that, we first need to reformulate what we want to show. Using {prf:ref}`Def:PropertiesOfSequences:Affirming`, we see that we must show that $a_{n+1} > a_n$ for all $n=1,2,3,\ldots$. So our statement $S_n$ is $a_{n+1} > a_n$.

First, consider $n=1$. Then the statement $S_1$ is $a_2>a_1$. Because we have $a_1=1$ and $a_2=3$, we also have $a_2-a_1=3-2=1$ and therefore $a_2>a_1$. This means our first statement $S_1$ is true.

Second, assume that the statement $S_k$ given by $a_{k+1} > a_k$ is true for some $k$. Then we must show that the statement $S_{k+1}$ given by $a_{k+2} > a_{k+1}$ is also true, which we will do next:

$$
a_{k+2}=\frac{1}{2}(a_{k+1}+5) > \frac{1}{2}(a_l+5)=a_{k+1}.
$$

As this shows that $S_{k+1}$ is true if $S_{k}$ is true, we conclude that our general statement $S_n$ is true for $n\geq 1$.

This shows that $a_{n+1} > a_n$ for all $n=1,2,3,\ldots$ and so the sequence is *strictly increasing*.
::::::
...
We want to prove that $b_{n+1} < b_n$ for all $n=1,2,3,\ldots$.

We again use the {prf:ref}`principle of mathematical induction <Thm:Algebra:MathematicalInduction>`.

For $n=1$ we have $b_2=4<10=b_1$. So the first condition of {prf:ref}`Thm:Algebra:MathematicalInduction` is satisfied.

Now, suppose that $b_{k+1} < b_k$ for a certain value of $k$, then we have

$$
b_{k+2}=\sqrt{b_{k+1}+6} < \sqrt{b_k+6}=b_{k+1}.
$$

This shows that $b_{n+1} < b_n$ for all $n=1,2,3,\ldots$ and so the sequence is *strictly decreasing*.
::::::

Even if a sequence is strictly increasing, strictly decreasing, non-increasing or non-decreasing, this does not mean that a sequence will attain a very positive or a very negative value. This concept is called the boundedness of sequences, which we will discuss next. 
...
We investigate whether the sequence $\{a_n\}_{n=1}^{\infty}$ with $a_n=\dfrac{n}{n+1}$ has bounds.
...
Now consider the sequence $\{b_n\}_{n=1}^{\infty}$ with $b_n=\dfrac{2n}{n^2+1}$ and its bounds.
...
Again we turn to a function. In this case we choose $g(x)=\dfrac{2x}{x^2+1}$ so that $b_n=g(n)$ as required. Then we have

$$
g'(x)=\frac{2(x^2+1)-2x\cdot2x}{(x^2+1)^2}=\frac{2(1-x^2)}{(x^2+1)^2},
$$

which is negative for $x>1$. This implies that $g$ is decreasing on $(1,\infty)$. Further we have $g(1)=1$ and $\lim\limits_{x\to\infty}g(x)=0$.

Hence, we have $0<g(x)\leq1$ for $x\geq1$.
...
Now suppose that $a_k<10$ for certain value of $k$, then we have
...
Again we use the {prf:ref}`principle of mathematical induction <Thm:Algebra:MathematicalInduction>` to show that $b_n>0$ for all $n=1,2,3,\ldots$.

For $n=1$ we have $b_1=10>0$.

Suppose that $b_k>0$ for certain value of $k$, then we have

$$
b_{k+1}=\sqrt{b_k+6}>\sqrt{0+6}=\sqrt{6}>0.
$$

This shows that $b_n>0$ for all $n=1,2,3,\ldots$.
::::::
