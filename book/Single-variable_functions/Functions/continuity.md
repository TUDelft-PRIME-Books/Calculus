...
Hier plaatje van de functie die $3+2x$ is voor $x\leq -2$, $x^2$ voor $-2<x<-1$, $-3$ op $x=-1$, $x+2$ voor $-1<x<1$ en $3-2x$ voor $x\geq 1$. In de applet en onderschrift NIET zeggen welke functie dit is.
...
Consider the function $f(x)=\left\{\begin{array}{ll}2^x+b&\mathrm{if }\,x< 2,\\ 2b+10&\mathrm{ if }\,x=2,\\ x^2-3x&\mathrm{if }\,x>2.\end{array}\right.$ 
...
$$
 \lim\limits_{x\rightarrow2^-}f(x)=\lim\limits_{x\rightarrow2^-}2^x+b=2^2+b=4+b,\qquad \lim\limits_{x\rightarrow2^+}f(x)=\lim\limits_{x\rightarrow2^+}x^2-3x=2^2-3\cdot 2=-2,\qquad f(2)=2b+10
$$
...
Hier nog een plaatje van deze functie met een slider van wat b is.
::::::
...
Hier nog een grafische weergave van de IMT. Ik wil twee plaatjes naast elkaar van de functie $f(x)=\cos(x)+\sin(x)$ op het interval $[0,\pi]$. Graag niet aangeven welke functie dit is en $0$ aangeven als $a$ en $\pi$ aangeven als $b$. In beide plaatjes worden worden de lijnen $y=\frac{1}{2}$ en $y=-\frac{1}{2}$ geplot, aangegeven als $f(a)$ resp $f(b)$. In het linkerplaatje de lijn $y=\frac{1}{4}$, aangegeven als $d$, met de bijbehorende verticale lijn, aangegeven als $c$. In het rechterplaatje de lijn $y=0.7$, aangegeven als $d$, met de twee bijbehorende verticale lijnen, aangegeven met $c_1$ and $c_2$. 
...
Hier een plaatje van deze functie tussen $0$ en $1$, met de horizontale lijnen op $-4$ en $1$ zichtbaar, net als de verticale lijn rond de oplossing $\approx 0.730154$
...
The assumption that the function $f$ in the Intermediate Value Theorem is continuous is essential. Indeed, consider the function $f(x)=\left\{\begin{array}{ll}x+3,&\mathrm{if }\,x\leq 1\\
2x+4,&\mathrm{if }\,x>1\end{array}\right.$
...
It is also important that the function is defined on each point in the closed interval $[a,b]$. Indeed, consider the function $g(x)=\frac{1}{x}$. Then $g$ is continuous on its maximal domain, which does not include $x=0$. Note that $f(-1)=-1$ and $f(1)$. However, there is no $x$ in between $-1$ and $1$ with $f(x)=0$.

Hier nog twee plaatjes met de grafieken van deze functies, met de lijnen $y=5$ resp. $y=0$ erin aangegeven.
::::::
