...
Solution. We need not to evaluate the integral, which would be impossible. However, application of the fundamental theorem of calculus leads to the immediate answer $e^{x^2}$.
::::::

::::::{prf:Example}
Find $\displaystyle\frac{d}{dx}\int_0^{\sqrt{x}}\cos(t^2)\,dt$.

Solution. Now we need to apply the chain rule:
```{math}
\frac{d}{dx}\int_0^{\sqrt{x}}\cos(t^2)\,dt=\sin\left((\sqrt{x})^2\right)\cdot\frac{d}{dx}(\sqrt{x})=\frac{\sin(x)}{2\sqrt{x}}.
```
::::::
...
However, in some applications functions appear, which are defined in terms of an integral that cannot be evaluated. Even though the integrand is continuous and an antiderivative exists, it is sometimes impossible to write this in terms of classical functions. In the examples below we deal with some of these functions. Although the integral cannot be evaluated, the fundamental theorem of calculus can be used to find properties of the function.

::::::{prf:Example}
:label: Ex:Integration:ErrorFucntion
...
::::::{prf:Example}
:label: Ex:Integration:FresnelIntegrals
